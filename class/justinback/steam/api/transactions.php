...
class transactions {
    ...
    public function AdjustAgreement($sNextProcessDate) {
        ...
        if ($CURLResponseCode != 200) {
            if ($CURLResponseCode == 400) {
                throw new \justinback\steam\exceptions\SteamRequestParameterException("The Agreement ID or another parameter is invalid!");
            }
        ...
        throw new \justinback\steam\exceptions\SteamRequestException(json_encode($CURLResponse->response->error));
    }
    ...
    public function RefundTxn() {
        ...
        if ($CURLResponseCode != 200) {
            if ($CURLResponseCode == 400) {
                throw new \justinback\steam\exceptions\SteamRequestParameterException("The Order ID or another parameter is invalid!");
            }
            if ($CURLResponseCode == 401) {
                throw new \justinback\steam\exceptions\SteamException("App ID or API Key is invalid.");
            }
    ...
    public function QueryTxn() {
        ...
        if ($CURLResponseCode != 200) {
            if ($CURLResponseCode == 400) {
                throw new \justinback\steam\exceptions\SteamRequestParameterException("The Order ID, Transaction ID or another parameter is invalid!");
            }
            if ($CURLResponseCode == 401) {
                throw new \justinback\steam\exceptions\SteamException("App ID or API Key is invalid.");
            }
    ...
    public function GetUserAgreementInfo() {
        ...
        if ($CURLResponseCode != 200) {
            if ($CURLResponseCode == 400) {
                throw new \justinback\steam\exceptions\SteamRequestParameterException("The Steam ID or another parameter is invalid!");
            }
            if ($CURLResponseCode == 401) {
                throw new \justinback\steam\exceptions\SteamException("App ID or API Key is invalid.");
            }
    ...
    public function GetReport($sTime, $iMaxResults = 1000, $sType = "GAMESALES", $bRawOutput = false) {
        ...
        foreach ($CURLResponse->response->params->orders as $Key => $Order) {
            ...
            $Transaction = new \justinback\steam\api\transactions($this->testing, $this->key, $this->game, $Order->steamid);
            $Transaction->agreementid = $Order->agreementid;
            $Transaction->orderid = $Order->orderid;
            $Transaction->transid = $Order->transid;


            $Object->Player = new \justinback\steam\api\player($this->key, $this->game, $Order->steamid);
