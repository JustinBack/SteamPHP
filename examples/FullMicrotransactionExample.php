...
// Here we do a global try catch, you can of course do this for each function individually!
...
try {
    ...
    // A parameter supplied is invalid
    print("<pre>" . print_r($ex, true) . "</pre>");
} catch (\justinback\steam\exceptions\SteamException $ex) {
    // App ID or the API Key is invalid.
    print("<pre>" . print_r($ex, true) . "</pre>");
} catch (\justinback\steam\exceptions\SteamIDInvalidException $ex) {
