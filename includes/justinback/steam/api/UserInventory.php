 ...
 */
class UserInventory implements \justinback\steam\interfaces\IUserInventory {

    use \justinback\steam\Hook;
    ...
    public function AddItem($sItemDefId, $sItemPropsJson = null, $bNotify = false, $sRequestId = null): \justinback\steam\api\item {
        ...
        //$this->send("beforeInventoryAddItem"ss);
        ...
        foreach ($oAddItem as $oResponse) {
            if (isset($oResponse->error)) {
                throw new \justinback\steam\exceptions\SteamRequestParameterException("A supplied itemdefid is invalid. " . $oResponse->error);
            }
        ...
        throw new \justinback\steam\exceptions\SteamRequestParameterException("The Itemdefid is invalid!");
    }
    ...
    public function Consolidate($aItemDefId, $bForce = false): array {
        ...
        foreach (json_decode($CURLResponse->response->item_json) as $oResponse) {
            ...
            array_push($aArray, new \justinback\steam\api\item($this->key, $this->game, $this->steamid, $oResponse->itemid, $oResponse->quantity, $oResponse->itemdefid, $oResponse->acquired, $oResponse->state, $oResponse->origin, $oResponse->state_changed_timestamp));
        }

        if (count($aArray) === 0) {
            throw new \justinback\steam\exceptions\SteamException("A supplied itemdefid is invalid, the user does not have a non stacked item in the inventory!");
        }
    ...
    public function GetInventory(): array {
        ...
        if ($CURLResponseCode != 200) {
            if ($CURLResponseCode == 400) {
                throw new \justinback\steam\exceptions\SteamException("Steam ID is invalid!");
            }
            if ($CURLResponseCode == 401) {
                throw new \justinback\steam\exceptions\SteamException("App ID or API Key is invalid.");
            }
        ...
        if (count($aArray) === 0) {
            throw new \justinback\steam\exceptions\SteamEmptyException("The inventory is empty!");
        }
    ...
    public function GetItemDefs(): array {
        ...
        if (count($aArray) === 0) {
            throw new \justinback\steam\exceptions\SteamEmptyException("No item definitions defined!");
        }
